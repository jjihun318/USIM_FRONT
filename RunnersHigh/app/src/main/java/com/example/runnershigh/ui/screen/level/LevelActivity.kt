...
class LevelActivity : AppCompatActivity(), CoroutineScope { // 파일명과 클래스명 변경됨
    ...
    // 🔧 더미 / 실제 API 전환 스위치 (이 부분을 false로 바꾸면 API 통신 시작)
    ...
    override fun onResume() {
        super.onResume()
        val currentUserId = 123 // 실제 로그인된 사용자 ID로 대체 필요
    ...
    private fun initViews() {
        // 레벨/EXP 카드
        levelTitle = findViewById(R.id.tv_level) // XML에서 Level 1 같은 텍스트뷰 ID로 가정
    ...
    private fun loadDummyData() {
        // 1. 더미 레벨 데이터
        val dummyLevel = UserLevel(123,4, 780) // Lv 4, 780 EXP
        updateLevelUI(dummyLevel)

        // 2. 더미 컨디션 데이터
        val dummyCondition = UserCondition(conditionLevel = "매우 좋음")
        updateConditionUI(dummyCondition)

        // 3. 더미 미션 데이터
        val dummyMissions = listOf(
            Mission(
                missionName = "주 3회 러닝",
                missionDescription = "이번 주 3회 러닝 도전",
                expPoints = 50,
                progressStatus = "2 / 3",
                missionCategory = "주간",
                gaugeRatio = 66
            ...
            Mission(
                missionName = "5km 완주",
                missionDescription = "한 번에 5km 이상 달리기",
                expPoints = 100,
                progressStatus = "1 / 1",
                missionCategory = "누적",
                gaugeRatio = 100 // 완료된 미션
            ),
            Mission(
                missionName = "총 100km 달성",
                missionDescription = "평생 누적 러닝 거리",
                expPoints = 200,
                progressStatus = "85 / 100",
                missionCategory = "평생",
                gaugeRatio = 85
            )
        )
        updateMissionUI(dummyMissions)

        Log.d("DUMMY_DATA", "더미 데이터로 UI를 업데이트했습니다.")
    }
    ...
    private fun fetchUserLevel(userId: Int) {
        launch(Dispatchers.IO) {
            try {
                val response = ApiClient.userService.getUserLevel(userId)
    ...
    private fun fetchUserMissions(userId: Int) {
        launch(Dispatchers.IO) {
            try {
                val response = ApiClient.userService.getUserMissions(userId)
    ...
    private fun fetchUserCondition(userId: Int) {
        launch(Dispatchers.IO) {
            try {
                val response = ApiClient.userService.getUserCondition(userId)
    ...
    private fun updateLevelUI(userLevel: UserLevel?) {
        ...
        expText.text = "${currentLevelExp}/${EXP_PER_LEVEL} EXP"
        // 남은 EXP 텍스트도 업데이트 가능: tv_exp_remaining.text = "-${EXP_PER_LEVEL - currentLevelExp} EXP"
    }
    ...
    private fun updateMissionUI(missions: List<Mission>?) {
        ...
        missions.forEachIndexed { index, mission ->
            if (index < 3) { // 최대 3개의 미션만 처리
                ...
                progressText.text = mission.progressStatus

                progressBar.max = 100
                progressBar.progress = mission.gaugeRatio
                layout.visibility = View.VISIBLE

                if (mission.gaugeRatio >= 100) {
                    ...
                    exp.text = "+${mission.expPoints} EXP"
                }
            }
        }
