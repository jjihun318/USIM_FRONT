...
import com.example.runnershigh.data.repository.HealthRepository
...
class AuthViewModel : ViewModel() {
    ...
    private val healthRepository = HealthRepository(ApiClient.healthApi)
    ...
    /**
     * 러닝 목적 선택 시 ViewModel에 저장.
     * - 선택한 목적이 1개라면 listOf("diet") 처럼 넘겨주고
     * - 여러 개라면 listOf("diet", "marathon") 처럼 넘겨줄 수 있음.
     */
    ...
    fun signup() {
        ...
        if (purposeList.isEmpty()) {
            _signupUiState.update {
                it.copy(errorMessage = "러닝 목적을 먼저 선택해주세요.")
            }
            return
        }
        ...
        if (experienceCode.isNullOrBlank()) {
            _signupUiState.update {
                it.copy(errorMessage = "러닝 경험을 먼저 선택해주세요.")
            }
            return
        }
                            ...
                            if (!purposeUpdated) {
                                ...
                                _signupUiState.update {
                                    it.copy(
                                        ...
                                        errorMessage = "러닝 목적 저장 중 오류가 발생했습니다."
                                    )
                                }
                                return@launch
                            }
                            ...
                            if (!experienceUpdated) {
                                ...
                                _signupUiState.update {
                                    it.copy(
                                        isLoading = false,
                                        signupCompleted = false,
                                        errorMessage = "러닝 경험 저장 중 오류가 발생했습니다."
                                    )
                                }
                                return@launch
                            }
        ...
        viewModelScope.launch {
            ...
            when (val result = repository.login(
                ...
                is LoginResult.Success -> {
                    ...
                    val profileResult = repository.getProfile(result.userUuid)
                    val consented = profileResult.getOrNull()?.healthConnectConsented
                    _loginUiState.update {
                        it.copy(
                            isLoading = false,
                            loginCompleted = true,
                            successMessage = "로그인에 성공했습니다.",
                            healthConnectConsented = consented ?: false
                        )
                    }
                }
    ...
    fun syncHealthConnectConsent(consented: Boolean) {
        val userUuid = currentUserUuid ?: return
        val timestamp = LocalDateTime.now()
            .format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"))

        viewModelScope.launch {
            healthRepository.syncHealthConnectConsent(
                userUuid = userUuid,
                consented = consented,
                consentedAt = timestamp
            )

            _loginUiState.update { it.copy(healthConnectConsented = consented) }
        }
    }

    fun syncHealthData(healthData: com.example.runnershigh.data.remote.dto.HealthData) {
        viewModelScope.launch {
            healthRepository.syncHealthData(healthData)
        }
    }
}
