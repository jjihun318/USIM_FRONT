...
fun ConditionDetailScreen(
    ...
    val conditionScore = 72
    val conditionHistory = listOf(
        WeeklyConditionScore("1주 전", 65),
        WeeklyConditionScore("이번 주", 72)
    )
    val injuryData = listOf(
        InjuryData("무릎", 30, 4, "주의"),
        InjuryData("발목", 18, 2, "보통")
    )
    val paceDecline = listOf(
        PaceDeclineData("4–6km", SeverityLevel.MEDIUM, "6'05\"", "+10\""),
        PaceDeclineData("8–10km", SeverityLevel.HIGH, "6'25\"", "+25\"")
    )
    val hrvData = listOf(
        HRVData("10/20", 45),
        HRVData("10/21", 50),
    ...
    val weeklyScores = conditionHistory
    ...
    Column(
        ...
        Column(
            ...
            ConditionScoreCard(score = conditionScore, conditionHistory = conditionHistory)
            ...
            ComprehensiveEvaluation(
                evaluationText = "피로는 조금 쌓였지만 회복력은 양호한 편입니다.\n" +
                        "장거리 러닝 후 스트레칭과 충분한 수면을 유지해 주세요."
            )
        }
    }
}
